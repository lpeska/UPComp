...
/**
 * Description of Item_itemCF
 * ItemItemCF implements Item-based KNN. Method applies collaborative filtering on specified implicit and explicit preferences via stored ite-item similarities
 ...
 */
   ...
   protected function find_similar($allowedObjectList="", $method="") {
     ...
     if(!$eventsList) {//wrong query
         ...
         while( $record = /*mysql_fetch_array($data)*/ $database->getNextRow($eventsList) ) {
             //get previous value
            // print_r($record);
            // print_r($this->similarObjectsScore);
